...
def all_block_names(cfg):
    """List all block names in textual order."""
    return [b["name"] for b in cfg["blocks"]]

def entry_name(cfg):
    """Entry block name (or None)."""
    return cfg["cfg"].get("entry")

def idx2name(cfg):
    """Index→name array as built by build_cfg_for_function."""
    return cfg["cfg"]["idx2name"]

def rpo_idx(cfg):
    """List of block indices in reverse postorder from entry (reachable only)."""
    return cfg["cfg"].get("rpo_idx") or []

def rpo_names(cfg):
    """List of block names in RPO (reachable only)."""
    i2n = idx2name(cfg)
    return [i2n[i] for i in rpo_idx(cfg)]

def preds_by_name(cfg):
    """Map: block name -> list of predecessor names."""
    return cfg["cfg"].get("preds", {})

def succs_by_name(cfg):
    """Map: block name -> list of successor names."""
...
def linearize_cfg(cfg):
    ...
    # Any block that is a jump/branch target needs a label. Include targets
    # referenced directly by terminators' labels, not only those present in
    # the static edges map (useful when CFG is modified without refreshing edges).
    target_labels = set()
    for src in order:
        for dst in edges.get(src, []):
            target_labels.add(dst)
    for name in [b for b in order]:
        body = block_by_name[name]["instrs"]
        if body and body[-1].get("op") in {"br", "jmp"}:
            for lab in body[-1].get("labels", []) or []:
                target_labels.add(lab)
